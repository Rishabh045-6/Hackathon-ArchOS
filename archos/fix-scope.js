...
const oldCode = `const USERS_DATA = (serverState?.users || []).length > 0 ? serverState.users.map((u: any) => ({
    name: u.user.name,
    email: u.user.email,
    role: u.role,
...
function UsersScreen({ members = [], roles = [], onInvite, serverState }: { members: any[], roles: any[], onInvite: (email: string, roleId: string) => void, serverState?: any }) {`;

const newCode = `function UsersScreen({ members = [], roles = [], onInvite, serverState }: { members: any[], roles: any[], onInvite: (email: string, roleId: string) => void, serverState?: any }) {
...
console.log('Fixed USERS_DATA scope');
