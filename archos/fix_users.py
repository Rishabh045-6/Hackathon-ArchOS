...
# Replace UsersScreen with a dynamic one
# We need to find the function UsersScreen() { ... }
import re
users_screen_regex = re.compile(r'function UsersScreen\(\) \{.*?\}\n', re.DOTALL)
              ...
              <div className="text-[12px] text-[#9E9A95] flex items-center relative">
                {isAdmin ? (
                  <select 
                    ...
                    className="bg-transparent border border-transparent hover:border-[#E5E1D9] focus:border-[#B07245] rounded px-1 py-0.5 outline-none text-[#1A1918] cursor-pointer"
                  >
...
content = users_screen_regex.sub(new_users_screen, content)

# Update the call to UsersScreen in renderScreen
content = content.replace("<UsersScreen />", "<UsersScreen serverState={serverState} />")
